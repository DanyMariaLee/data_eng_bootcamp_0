...
```scala
val df1 = spark.read.format("csv").option("header", "true").load("dbfs:/FileStore/shared_uploads/YOUR_EMAIL/input_data.csv")
df1.show
```
...
To have a clear idea of what columns we have we can call function 
...
.withColumn("t", col("Total").cast("decimal"))
.drop("Total")
.withColumnRenamed("t", "Total")
.groupBy("CustomerID")
.sum("Total")
...
Let's read this data. Hint: after drag and drop of the file into our notebook, the line generated by notebook will name our val df1 again, we need to rename it into df2 to avoid the confusion.

```scala
val df2 = spark.read.format("csv").option("header", "true").load("dbfs:/FileStore/shared_uploads/YOUR_EMAIL/reference_data.csv")
df2.show
```
...
*this is the question from Christmas, the answer below is for you to check your solution result.
